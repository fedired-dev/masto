...
module Fedired::CLI
  class Domains < Base
    ...
    long_desc <<-LONG_DESC
      ...
      `https://fedired-bar.com/users/foo`, would be purged by either
      `tootctl domains purge bar.com` or `tootctl domains purge --by-uri fedired-bar.com`.
    ...
    long_desc <<-LONG_DESC
      Crawl the fediverse by using the Fedired REST API endpoints that expose
