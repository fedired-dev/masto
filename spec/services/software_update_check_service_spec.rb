...
RSpec.describe SoftwareUpdateCheckService do
  ...
  shared_examples 'when the feature is enabled' do
    let(:full_update_check_url) { "#{update_check_url}?version=#{Fedired::Version.to_s.split('+')[0]}" }
  ...
  context 'when update checking is disabled' do
    around do |example|
      original = Rails.configuration.x.fedired.software_update_url
      Rails.configuration.x.fedired.software_update_url = ''
      example.run
      Rails.configuration.x.fedired.software_update_url = original
    end
  ...
  context 'when using the default update checking API' do
    let(:update_check_url) { 'https://api.fedired.com/update-check' }
  ...
  context 'when using a custom update check URL' do
    ...
    around do |example|
      original = Rails.configuration.x.fedired.software_update_url
      Rails.configuration.x.fedired.software_update_url = 'https://api.example.com/update_check'
      example.run
      Rails.configuration.x.fedired.software_update_url = original
    end
