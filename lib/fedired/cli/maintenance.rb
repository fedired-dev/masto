...
module Fedired::CLI
  class Maintenance < Base
    ...
    long_desc <<~LONG_DESC
      ...
      Fedired has to be stopped to run this task, which will take a long time and may be destructive.
    LONG_DESC
    def fix_duplicates
      verify_system_ready!
    ...
    def verify_schema_version!
      if migrator_version < MIN_SUPPORTED_VERSION
        fail_with_message <<~ERROR
          ...
          Please update to at least Fedired 3.0.0 before running this script.
        ERROR
    ...
    def verify_sidekiq_not_active!
      fail_with_message 'It seems Sidekiq is running. All Fedired processes need to be stopped when using this script.' if Sidekiq::ProcessSet.new.any?
    end

    def verify_backup_warning!
      say 'This task will take a long time to run and is potentially destructive.', :yellow
      say 'Please make sure to stop Fedired and have a backup.', :yellow
