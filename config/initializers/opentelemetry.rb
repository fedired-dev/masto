...
if ENV.keys.any? { |name| name.match?(/OTEL_.*_ENDPOINT/) }
  ...
  OpenTelemetry::SDK.configure do |c|
    ...
    prefix    = ENV.fetch('OTEL_SERVICE_NAME_PREFIX', 'fedired')
    ...
    c.service_version = Fedired::Version.to_s

    if Fedired::Version.source_commit.present?
      c.resource = OpenTelemetry::SDK::Resources::Resource.create(
        'vcs.repository.ref.revision' => Fedired::Version.source_commit,
        'vcs.repository.url.full' => Fedired::Version.source_base_url
      )
    end
  end
end
