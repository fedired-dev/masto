...
module Fedired::CLI
  class Media < Base
    ...
    def refresh
      ...
      scope = scope.where('media_attachments.id > ?', Fedired::Snowflake.id_at(options[:days].days.ago, with_random: false)) if options[:days].present?
    ...
    desc 'usage', 'Calculate disk space consumed by Fedired'
    def usage
      print_table [
