...
RSpec.describe Request do
  ...
  describe '#perform' do
    ...
    context 'with private host' do
      ...
      it 'raises Fedired::ValidationError' do
        resolver = instance_double(Resolv::DNS)
        ...
        expect { subject.perform }.to raise_error Fedired::ValidationError
      end
    end
  end
  ...
  describe "response's body_with_limit method" do
    it 'rejects body more than 1 megabyte by default' do
      ...
      expect { subject.perform(&:body_with_limit) }.to raise_error(Fedired::LengthValidationError, 'Body size exceeds limit of 1048576')
    end
    ...
    it 'rejects body by given size' do
      stub_request(:any, 'http://example.com').to_return(body: SecureRandom.random_bytes(2.kilobytes))
      expect { subject.perform { |response| response.body_with_limit(1.kilobyte) } }.to raise_error(Fedired::LengthValidationError, 'Body size exceeds limit of 1024')
    end
    ...
    it 'rejects too large monolithic body' do
      ...
      expect { subject.perform(&:body_with_limit) }.to raise_error(Fedired::LengthValidationError, 'Content-Length 2097152 exceeds limit of 1048576')
    end
