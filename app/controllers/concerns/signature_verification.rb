...
module SignatureVerification
  ...
  def signed_request_actor
    ...
    # Compatibility quirk with older Fedired versions
    ...
    fail_with! e.message
  rescue *Fedired::HTTP_CONNECTION_ERRORS => e
    fail_with! "Failed to fetch remote data: #{e.message}"
  rescue Fedired::UnexpectedResponseError
  ...
  def verify_signature_strength!
    raise SignatureVerificationError, 'Fedired requires the Date header or (created) pseudo-header to be signed' unless signed_headers.include?('date') || signed_headers.include?('(created)')
    raise SignatureVerificationError, 'Fedired requires the Digest header or (request-target) pseudo-header to be signed' unless signed_headers.include?(Request::REQUEST_TARGET) || signed_headers.include?('digest')
    raise SignatureVerificationError, 'Fedired requires the Host header to be signed when doing a GET request' if request.get? && !signed_headers.include?('host')
    raise SignatureVerificationError, 'Fedired requires the Digest header to be signed when doing a POST request' if request.post? && !signed_headers.include?('digest')
  end
  ...
  def verify_body_digest!
    ...
    raise SignatureVerificationError, "Fedired only supports SHA-256 in Digest header. Offered algorithms: #{digests.map(&:first).join(', ')}" if sha256.nil?
    ...
    signed_headers.map do |signed_header|
      ...
      when Request::REQUEST_TARGET
        if include_query_string
          ...
          # Current versions of Fedired incorrectly omit the query string from the (request-target) pseudo-header.
  ...
  rescue Fedired::PrivateNetworkAddressError => e
    raise SignatureVerificationError, "Requests to private network addresses are disallowed (tried to query #{e.host})"
  rescue Fedired::HostValidationError, ActivityPub::FetchRemoteActorService::Error, ActivityPub::FetchRemoteKeyService::Error, Webfinger::Error => e
  ...
  def actor_refresh_key!(actor)
    ...
    raise SignatureVerificationError, "Requests to private network addresses are disallowed (tried to query #{e.host})"
  rescue Fedired::HostValidationError, ActivityPub::FetchRemoteActorService::Error, Webfinger::Error => e
    raise SignatureVerificationError, e.message
  end
end
