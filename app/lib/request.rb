...
class Request
  ...
  def initialize(verb, url, **options)
    ...
    raise Fedired::HostValidationError, 'Instance does not support hidden service connections' if block_hidden_service?
  ...
  def set_common_headers!
    ...
    @headers['User-Agent']      = Fedired::Version.user_agent
  ...
  module ClientLimit
    ...
    def body_with_limit(limit = 1.megabyte)
      ...
      raise Fedired::LengthValidationError, "Body size exceeds limit of #{limit}" if contents.bytesize > limit

      contents
    end

    def require_limit_not_exceeded!(limit)
      raise Fedired::LengthValidationError, "Content-Length #{content_length} exceeds limit of #{limit}" if content_length.present? && content_length > limit
    end
  end
  ...
  class Socket < TCPSocket
    class << self
      ...
      def check_private_address(address, host)
        ...
        raise Fedired::PrivateNetworkAddressError, host if PrivateAddressCheck.private_address?(addr)
      end
    end
  end
