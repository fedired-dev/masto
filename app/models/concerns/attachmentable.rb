...
module Attachmentable
  ...
  def check_image_dimension(attachment)
    ...
    if attachment.content_type == 'image/gif' && width * height > GIF_MATRIX_LIMIT
      raise Fedired::DimensionsValidationError, "#{width}x#{height} GIF files are not supported"
    elsif width * height > MAX_MATRIX_LIMIT
      raise Fedired::DimensionsValidationError, "#{width}x#{height} images are not supported"
    end
  end
