...
RSpec.describe Account::FinderConcern do
  ...
  describe 'remote finders' do
    let!(:account) { Fabricate(:account, username: 'Alice', domain: 'fedired.social') }
    ...
    describe '.find_remote' do
      ...
      it 'returns nil when username does not match' do
        expect(Account.find_remote('a_ice', 'fedired.social')).to be_nil
      end
    ...
    describe '.find_remote!' do
      ...
      it 'raises on non-matching result' do
        expect { Account.find_remote!('missing', 'fedired.host') }.to raise_error(ActiveRecord::RecordNotFound)
      end
