...
class AnnualReport::Percentiles < AnnualReport::Source
  def self.prepare(year)
    AnnualReport::StatusesPerAccountCount.connection.exec_query(<<~SQL.squish, nil, [year, Fedired::Snowflake.id_at(DateTime.new(year).beginning_of_year), Fedired::Snowflake.id_at(DateTime.new(year).end_of_year)])
