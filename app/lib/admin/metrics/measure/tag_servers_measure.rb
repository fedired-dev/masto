...
class Admin::Metrics::Measure::TagServersMeasure < Admin::Metrics::Measure::BaseMeasure
  ...
  def perform_total_query
    tag.statuses.where('statuses.id BETWEEN ? AND ?', Fedired::Snowflake.id_at(@start_at, with_random: false), Fedired::Snowflake.id_at(@end_at, with_random: false)).joins(:account).count('distinct accounts.domain')
  end

  def perform_previous_total_query
    tag.statuses.where('statuses.id BETWEEN ? AND ?', Fedired::Snowflake.id_at(@start_at - length_of_period, with_random: false), Fedired::Snowflake.id_at(@end_at - length_of_period, with_random: false)).joins(:account).count('distinct accounts.domain')
  end
  ...
  def earliest_status_id
    Fedired::Snowflake.id_at(@start_at.beginning_of_day, with_random: false)
  end

  def latest_status_id
    Fedired::Snowflake.id_at(@end_at.end_of_day, with_random: false)
  end
