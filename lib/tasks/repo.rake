...
namespace :repo do
  ...
  task :authors do
    ...
    file << <<~HEADER
      ...
      Fedired is available on [GitHub](https://github.com/#{REPOSITORY_NAME})
    ...
    file << <<~FOOTER
      ...
      This document is provided for informational purposes only. Since it is only updated once per release, the version you are looking at may be currently out of date. To see the full list of contributors, consider looking at the [git history](https://github.com/fedired/fedired/graphs/contributors) instead.
    FOOTER
  end
  ...
  task check_locales_files: :environment do
    ...
    missing_json_files = I18n.available_locales.reject { |locale| Rails.root.join('app', 'javascript', 'fedired', 'locales', "#{locale}.json").exist? }
