...
namespace :tests do
  namespace :migrations do
    ...
    task check_database: :environment do
      ...
      unless PreviewCard.where(id: PreviewCardsStatus.where(status_id: 12).select(:preview_card_id)).pluck(:url) == ['https://joinfedired.org/']
    ...
    task populate_v2: :environment do
      ...
      ActiveRecord::Base.connection.execute(<<~SQL)
        ...
        INSERT INTO "statuses"
          (id, account_id, text, created_at, updated_at)
        VALUES
          (12, 1, 'check out https://joinfedired.org/', now(), now());
        ...
        VALUES
          (1, 'https://joinfedired.org/', 'Fedired - Decentralized social media', now(), now());
