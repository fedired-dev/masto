...
RSpec.describe User do
  ...
  describe 'allowlist integration' do
    around do |example|
      ...
      Rails.configuration.x.email_domains_allowlist = 'fedired.space'
    ...
    it 'allows a user to be created when their email is on the allowlist' do
      user = described_class.new(email: 'foo@fedired.space', account: account, password: password, agreement: true)
      expect(user).to be_valid
    end

    it 'does not allow a user with an email subdomain included on the top level domain allowlist to be created' do
      user = described_class.new(email: 'foo@fedired.space.userdomain.com', account: account, password: password, agreement: true)
      expect(user).to_not be_valid
    end
    ...
    context 'with a subdomain on the denylist' do
      ...
      it 'does not allow a user to be created with an email subdomain on the denylist even if the top domain is on the allowlist' do
        Rails.configuration.x.email_domains_denylist = 'denylisted.fedired.space'

        user = described_class.new(email: 'foo@denylisted.fedired.space', account: account, password: password)
        expect(user).to_not be_valid
      end
    end
  end
