...
class FetchLinkCardService < BaseService
  ...
  rescue *Fedired::HTTP_CONNECTION_ERRORS, Addressable::URI::InvalidURIError, Fedired::HostValidationError, Fedired::LengthValidationError, Encoding::UndefinedConversionError, ActiveRecord::RecordInvalid => e
  ...
  def html
    ...
    headers = {
      ...
      'User-Agent' => "#{Fedired::Version.user_agent} Bot",
    }
  ...
  def attempt_oembed
    ...
    when 'video'
      @card.width            = embed[:width].presence  || 0
      @card.height           = embed[:height].presence || 0
      @card.html             = Sanitize.fragment(embed[:html], Sanitize::Config::FEDIRED_OEMBED)
      @card.image_remote_url = (url + embed[:thumbnail_url]).to_s if embed[:thumbnail_url].present?
    when 'rich'
