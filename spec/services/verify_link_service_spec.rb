...
RSpec.describe VerifyLinkService do
    ...
    context 'when a link contains an <a> back' do
      let(:html) do
        <<~HTML
          <!doctype html>
          <body>
            <a href="#{ActivityPub::TagManager.instance.url_for(account)}" rel="me">Follow me on Fedired</a>
          </body>
        HTML
      end
    ...
    context 'when a link contains an <a rel="me noopener noreferrer"> back' do
      let(:html) do
        <<~HTML
          <!doctype html>
          <body>
            <a href="#{ActivityPub::TagManager.instance.url_for(account)}" rel="me noopener noreferrer" target="_blank">Follow me on Fedired</a>
          </body>
        HTML
      end
    ...
    context 'when link has no `href` attribute' do
      let(:html) do
        <<~HTML
          ...
          <body>
            <a rel="me" target="_blank">Follow me on Fedired</a>
          </body>
        HTML
      end
  ...
  context 'when given a remote account' do
    ...
    context 'when a link contains an <a> back' do
      let(:html) do
        <<~HTML
          <!doctype html>
          <body>
            <a href="https://profile.example.com/alice" rel="me">Follow me on Fedired</a>
          </body>
        HTML
      end
    ...
    context 'when the link contains a link with a missing protocol slash' do
      ...
      # 1. their fedired profile, which linked to github and the personal website
      # 2. their personal website correctly linking back to fedired
      # 3. a github profile that was linking to the personal website, but with
      #    a malformed protocol of http:/
      #
      # This caused link verification between the fedired profile and the
      # website to fail.
      #
