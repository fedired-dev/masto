      ...
      define_method(:"download_#{attachment_name}!") do |url = nil|
        ...
        begin
          Request.new(:get, url).perform do |response|
            raise Fedired::UnexpectedResponseError, response unless (200...300).cover?(response.code)

            public_send(:"#{attachment_name}=", ResponseWithLimit.new(response, limit))
          end
        rescue Fedired::UnexpectedResponseError, *Fedired::HTTP_CONNECTION_ERRORS => e
          Rails.logger.debug { "Error fetching remote #{attachment_name}: #{e}" }
          public_send(:"#{attachment_name}=", nil) if public_send(:"#{attachment_name}_file_name").present?
          raise e unless suppress_errors
        rescue Paperclip::Errors::NotIdentifiedByImageMagickError, Addressable::URI::InvalidURIError, Fedired::HostValidationError, Fedired::LengthValidationError, Paperclip::Error, Fedired::DimensionsValidationError, Fedired::StreamValidationError => e
          Rails.logger.debug { "Error fetching remote #{attachment_name}: #{e}" }
          public_send(:"#{attachment_name}=", nil) if public_send(:"#{attachment_name}_file_name").present?
        end
