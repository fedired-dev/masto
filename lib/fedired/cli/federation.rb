  ...
  module Federation
    ...
    included do
      ...
      long_desc <<~LONG_DESC
        ...
        running a Fedired server, as it leaves next to no cache behind on
        other servers.
      ...
      def self_destruct
        if SelfDestructHelper.self_destruct?
          say('Self-destruct mode is already enabled for this Fedired server', :green)
        ...
        say(<<~INSTRUCTIONS, :green)
          To switch Fedired to self-destruct mode, add the following variable to your evironment (e.g. by adding a line to your `.env.production`) and restart all Fedired processes:
