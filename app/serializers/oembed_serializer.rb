...
class OEmbedSerializer < ActiveModel::Serializer
  ...
  def html
    <<~HTML.squish
      <blockquote class="fedired-embed" data-embed-url="#{embed_short_account_status_url(object.account, object)}" style="#{INLINE_STYLES[:blockquote]}">
        <a href="#{short_account_status_url(object.account, object)}" target="_blank" style="#{INLINE_STYLES[:status_link]}">
          ...
          <div style="#{INLINE_STYLES[:div_view]}">View on Fedired</div>
        </a>
      </blockquote>
