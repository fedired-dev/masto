...
namespace :fedired do
  ...
  task :setup do
    ...
    # When the application code gets loaded, it runs `lib/fedired/redis_configuration.rb`.
    ...
    begin
      ...
      using_docker        = prompt.yes?('Are you using Docker to run Fedired?')
      ...
      loop do
        ...
        env['DB_NAME'] = prompt.ask('Name of PostgreSQL database:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'fedired_production'
          q.modify :strip
        end

        env['DB_USER'] = prompt.ask('Name of PostgreSQL user:') do |q|
          q.required true
          q.default using_docker ? 'postgres' : 'fedired'
          q.modify :strip
        end
      ...
      loop do
        ...
        env['SMTP_FROM_ADDRESS'] = prompt.ask('E-mail address to send e-mails "from":') do |q|
          q.required true
          q.default "Fedired <notifications@#{env['LOCAL_DOMAIN']}>"
          q.modify :strip
        end
        ...
        begin
          ...
          mail = ActionMailer::Base.new.mail(
            ...
            body: 'Fedired SMTP configuration works!'
          )
          mail.deliver
          break
        rescue => e
      ...
      env['UPDATE_CHECK_URL'] = '' unless prompt.yes?('Do you want Fedired to periodically check for important updates and notify you? (Recommended)', default: true)
      ...
      if prompt.yes?('Save configuration?')
        ...
        prompt.say "\n"
        if errors
          prompt.warn 'Your Fedired server is set up, but there were some errors along the way, you may have to fix them.'
        else
          prompt.ok 'All done! You can now power on the Fedired server 🐘'
        end
        prompt.say "\n"
  ...
  def generate_header(include_warning)
    default_message = "# Generated with fedired:setup on #{Time.now.utc}\n\n"
