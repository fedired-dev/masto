...
class SoftwareUpdateCheckService < BaseService
  ...
  def fetch_update_notices
    Request.new(:get, "#{api_url}?version=#{version}").add_headers('Accept' => 'application/json', 'User-Agent' => 'Fedired update checker').perform do |res|
      return Oj.load(res.body_with_limit, mode: :strict) if res.code == 200
    end
  rescue *Fedired::HTTP_CONNECTION_ERRORS, Oj::ParseError
    nil
  end
  ...
  def version
    @version ||= Fedired::Version.to_s.split('+')[0]
  end
